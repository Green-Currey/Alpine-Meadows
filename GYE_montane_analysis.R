...
gye <- readOGR('f:/GIS_data/GYE/Shapefile/GYE_WGS_OUTLINE.shp')
...
# dem <- raster('f:/GIS_data/GYE/Raster/30m_dem_GYE.tif')
# dem.gye <- crop(dem, gye)
# slope <- raster('f:/GIS_data/GYE/Raster/30m_slope_GYE.tif')
# slope.gye <- crop(slope, gye)
# LER <- raster('f:/GIS_data/GYE/Raster/1km_LER_GYE.tif')
...
# writeRaster(slope.gye, 'f:/GIS_data/GYE/Raster/30m_slope_GYE_cropped.tif', overwrite = T)
# writeRaster(ler.gye, 'f:/GIS_data/GYE/Raster/30m_LER_GYE_cropped.tif', overwrite = T)

# Reading in raster data --------------------------------------------------


dem <- raster('f:/GIS_data/GYE/Raster/30m_dem_GYE_cropped.tif')
slope <- raster('f:/GIS_data/GYE/Raster/30m_slope_GYE_cropped.tif')
ler <- raster('f:/GIS_data/GYE/Raster/30m_LER_GYE_cropped.tif')
d2 <- slope >= 2
d5 <- slope >= 5
...
gye.montane <- L1 + L2 + L3 + L4 + L5 + L6
writeRaster(gye.montane, 'f:/gis_data/gye/Raster/30m_montane_GYE_cropped.tif', overwrite = T)
gye.montane.m <- mask(gye.montane, gye)
writeRaster(gye.montane.m, 'f:/gis_data/gye/Raster/30m_montane_GYE_masked.tif', overwrite = T)
